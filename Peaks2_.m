...
% search the standard deviation of trekD noises 
    ...
    fprintf('Number of Good peaks before Double front search= %3.0f\n',GoodPeakN);
  ...
  %close(StFP);  
    ...
    S=PulseIMaxIdx-StartFitPoint*TauFitN;     %MK
    E=PulseIMaxIdx+EndFitPoint*TauFitN;%MK
    for i=1:4*TauFitN
        PulseInterpShifted=circshift(PulseInterp(:,2),-2*TauFitN-1+i);
        FitPulse(1:FitN,i)=PulseInterpShifted(S:TauFitN:E);%MK
        %FitPulseShort(1:FitN-EndFitPoint,i)=PulseInterpShifted(S:TauFitN:PulseIMaxIdx);      %MK
        Sums2(i)=sum(FitPulse(:,i));
        Sums3(i)=FitPulse(:,i)'*FitPulse(:,i);
        Sums2Short(i)=sum(FitPulse(1:FitN-EndFitPoint,i));
        Sums3Short(i)=FitPulse(1:FitN-EndFitPoint,i)'*FitPulse(1:FitN-EndFitPoint,i);
    end;
    ...
    peaks=[];Khi2Fin=[];
    NPeaks=0;
    StartF10=StartFitPoint*TauFitN*10;
    while i<size(PeakInd,1)
       ...
       if ShortFit   
            ...
            Sum2=Sums2Short;   Sum3=Sums3Short;            
       else
            FitNi=FitN;  
            Sum2=Sums2;   Sum3=Sums3;            
       end;
       EndF10=(FitNi-1)*10*TauFitN;
       FitPoints(1:FitNi,:)=trekMinus(PeakInd(i)-StartFitPoint:PeakInd(i)-StartFitPoint+FitNi-1,:);
       for k=1:4*TauFitN;
           Sum1=FitPoints(:,2)'*FitPulse(1:FitNi,k);
           A=(Sum1-B*Sum2(k))/Sum3(k);
           Khi2Fit(k)=(FitPoints(:,2)-A*FitPulse(1:FitNi,k)-B)'*(FitPoints(:,2)-A*FitPulse(1:FitNi,k)-B);
            %    plot(FitPoints(:,1),A*FitPulse(1:FitNi,k)+B,'-r.');
           %      pause;
       end;
       [MinKhi2(i),MinKhi2Idx(i)]=min(Khi2Fit);
       if (MinKhi2Idx(i)>2)&(MinKhi2Idx(i)<4*TauFitN-2)
           PolyKhi2=polyfit((MinKhi2Idx(i)-2:1:MinKhi2Idx(i)+2),...
                     Khi2Fit(MinKhi2Idx(i)-2:MinKhi2Idx(i)+2)/(FitNi*StdVal^2),2);
           ...
           FitPulseFin(1:FitNi,1)=...
                 PulseInterp10Shifted(PulseI10MaxIdx-StartF10:TauFitN*10:PulseI10MaxIdx-StartF10+EndF10);
        ...
        if (Khi2Fin(i))>0
            if  Khi2Fin(i)/(FitNi*StdVal^2)<Khi2Thr  
                ...
                if A*PulseMax>Threshold
                    ...
                    peaks(NPeaks,6)=peaks(NPeaks,5)*0.5;                                         %FrontCharge
                    peaks(NPeaks,7)=MaxFront+MaxTail;                                            % peak or front duration (depending on FrontCharge)
                    peaks(NPeaks,8)=1;                                                           % number of combined peaks in one peak
                end;                       
 ...
 clear k i hp SF VisiblePeakInd TrekPlot0  TailIdx Tail;
 clear Sum3 Sum2 Sum1 StandardPulsesNorm StandardPulses StandardPulse;
 clear StandardPeaksN StandardPeaks StPeakN ;
 clear Khi2Fit S RangeSP Range PulseOverThr PolyTail PolyFitLog PeakVal PeakStart;
 clear PeakSpanInd PeakOnFrontInd PeakN PeakInd PeakEnd OutRangeN NumPeaks NumGoodPeaks;
 clear MinStPeakIdx MinStPeak MinKhi2Idx MinKhi2 MinInterval M Ind Idx;
 clear GoodPeakN GoodPeakInd GoodPeakVal FrontSignalN FitNOld FitLog FitIdx FPS1  EndIdx E;
 clear DoubleFrontSize DoubleFrontInd A B MaxStPeak MaxStPeakIdx MaxSPInd MaxIndSP MaxSP MinSP MinSPInd;
...
while DeltaNPeaks>0.1*NPeaks
  ...
  if SecondPassFull
    clear ThresholdD Threshold StdValD StdVal StartSignal EndSignal SlowN SlowInd Slow SizeMoveToSignal SignalN;
    % search the signal pedestal and standard deviation 
    NoiseArray=logical(ones(trekSize,1));  % first, all measurements are considered as noise
    [MeanVal,StdVal,PeakPolarity,NoiseArray]=MeanSearch(trek,OverSt,NoiseArray,0,0); 
    %fprintf(' PeakPolarity = %2.0f\n', PeakPolarity);
    ...
    clear MoveToNoise SizeMoveToNoise Range;
    ...
    %fprintf('Pause. look at the figure and press any key\n');
    %pause;
    clear StartSignal EndSignal SignalN NumPeaks; 
    ...
    while i<size(PeakInd,1)
       ...
       Khi2Fin(i)=-1;
        ...
        [EndIdx,M]=min([PeakInd(i)+PulseN,trekSize]);
        %plot(trekMinus(PeakInd(i)-10:EndIdx,1),trekMinus(PeakInd(i)-10:EndIdx,2),'-b.');
        %plot(FitPoints(:,1),FitPoints(:,2),'ro');

       B=mean(trekMinus(PeakInd(i)-StartBFitPoint-BFitPointsN+1:PeakInd(i)-StartBFitPoint,2));
       ShortFit=not(isempty(find(PeakOnFrontInd==PeakInd(i))));
       if ShortFit
           FitNi=FitN-EndFitPoint;   Sum2=Sums2Short(k);        Sum3=Sums3Short(k);
       else
           FitNi=FitN;      Sum2=Sums2(k);        Sum3=Sums3(k);
       end;
       FitPoints(1:FitNi,:)=trekMinus(PeakInd(i)-StartFitPoint:PeakInd(i),:);
       for k=1:4*TauFitN;
           Sum1=sum(FitPoints(:,2).*FitPulse(1:FitNi,k));
           A=(Sum1-B*Sum2)/Sum3;
           Khi2Fit(k)=sum((FitPoints(:,2)-A*FitPulse(1:FitNi,k)-B).^2);
           %plot(FitPoints(:,1),A*FitPulse(1:FitNi,k)+B,'-r.');
           %pause;
       end;
       [MinKhi2(i),MinKhi2Idx(i)]=min(Khi2Fit);
       if (MinKhi2Idx(i)>2)&(MinKhi2Idx(i)<4*TauFitN-2)
           ...
           Sum1=sum(FitPoints(:,2).*FitPulseFin(:));
           Sum2=sum(FitPulseFin(:));
           Sum3=sum(FitPulseFin(:).^2);
           A=(Sum1-B*Sum2)/Sum3;
           Khi2Fin(i)=sum((FitPoints(:,2)-A*FitPulseFin(:)-B).^2);
       end;
  ...
  clear i k hp VisiblePeakInd TrekPlot0 Sum1 Sum2 Sum3 A B M E FitNi;
