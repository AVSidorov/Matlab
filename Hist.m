function HistA=Hist(FileName);
% HistA=Hist(FileName);  makes histograms from a peak file.
% The peraks are calculated by GetPeaks.m program
...
disp(['track duration = ',num2str(MeasureInterval),' us']);
disp(['The number of peaks = ',num2str(NPeaks)]);
...
subplot(2,2,1);  hold on; grid on; axis([0,max(MaxAmpl),1,1.2*MaxAmplN]); 
                 for n=1:TimeSpectralIntervalNum   
                   ...
                   text(max(MaxAmpl)/3,MaxAmplN*(1-(n-1)/TimeSpectralIntervalNum),num2str(n),'Color',cm(n,:));
                 end; 
                 set(gca,'YScale','log');                
