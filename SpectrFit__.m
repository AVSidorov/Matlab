function [K_W,K_Amp,Wb]=SpectrFit(Spectr,MaxwSp,Wmax,CalAmplt,CalAmplf,MeasAmplf);
...
%EndFitPntInd=find(spectr1(:,1)<=MaxwSp(FitI(end),1),1,'last');
EndFitPntInd=find(spectr1(:,1)<=MaxwSp(end,1),1,'last');
StFitPntInd=find(spectr1(:,1)>=MaxwSp(FitI(1),1),1,'first');
%FitEndI=MaxSpectrI+FitEndI;
MaxwFitLn=interp1(MaxwSp(:,1),MaxwSp(:,2),log(spectr1(StFitPntInd:EndFitPntInd,1)),'spline');
MaxwFit=exp(MaxwFitLn(1:end));
% StFitLnPntInd=EndFitPntInd+1;
% EndFitLnPntInd=size(spectr1,1);
% MaxwFitLn=Interp1(MaxwSp(:,1),log(MaxwSp(:,2)),spectr1(StFitLnPntInd:EndFitLnPntInd,1));
% MaxwFit=[MaxwFit(1:end);exp(MaxwFitLn(1:end))];

Amp=MaxSpectr/max(MaxwFit);
MaxwFit=MaxwFit*Amp;

%WbInd=find((spectr1(StFitPntInd:EndFitLnPntInd,2)-MaxwFit)>spectr1(StFitPntInd:EndFitLnPntInd,3));
...
%   plot([spectr1(StFitLnPntInd,1),spectr1(StFitLnPntInd,1)],[spectr1(1,2),max(spectr1(:,2))],'k','LineWidth',1);
%   plot([spectr1(EndFitLnPntInd,1),spectr1(EndFitLnPntInd,1)],[spectr1(1,2),max(spectr1(:,2))],'k','LineWidth',1);
...
% 
%         plot([x,x],[1,max(spectr1(:,2))],'c','LineWidth',2);
% 
%         disp('Input End Ln Fit Point Zoom figure if nessecary then press enter');
%         pause;
%         figure(trf);
%         [x,y]=ginput(1);
%         EndFitLnPntInd=find(abs(spectr1(:,1)-x)<=dW/2,1,'first');
%         if isempty(EndFitLnPntInd)
%             EndFitLnPntInd=StFitLnPntInd+1;
%         end;
% 
%         plot([x,x],[1,max(spectr1(:,2))],'m','LineWidth',2);
