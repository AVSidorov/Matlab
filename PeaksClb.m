...
Text=true;            % switch between text and binary input files
...
MaxTail=0.6;      % maximal peak tail, us
...
InterpN=8;        %number of extra intervals for intrpolation of Standard Pulse in fitting
FineInterpN=40;   %number of extra intervals for fine intrpolation of Standard Pulse in fitting
Khi2Thr=50;        %Sigma^2 threshold in pulsefiting;      
...
%remove points with the the instnces (important for calibration files)
trekD(:,1)=diff(trek(:,1));  bool=trekD==0; 
trek(bool,:)=[]; trekD(bool)=[];  clear bool; 
trekSize=size(trek(:,1),1);
trekD(:,2)=diff(trek(:,2)); 
trekD(end+1,1)=tau;  trekD(end+1,2)=trekD(end,2);
%remove rermove signal jumps when time jumps. (important for calibration files)
JumpInd=find(trekD(:,1)>2*tau);
% JumpVal=trek(JumpInd+1,2)-trek(JumpInd,2);
% JumpN=size(JumpInd,1);
% for i=2:JumpN; JumpVal(i)=JumpVal(i)+JumpVal(i-1);  end;   
% JumpCorr=zeros(trekSize,1);
% for i=1:JumpN-1; JumpCorr(JumpInd(i)+1:JumpInd(i+1))=JumpVal(i);end;
% JumpCorr(JumpInd(JumpN)+1:end)=JumpVal(JumpN);
% JumpFit=polyfit([1:trekSize]',JumpCorr,1);
% trek(:,2)=trek(:,2)-JumpCorr;
% trek(:,2)=trek(:,2)+JumpFit(1)*[1:trekSize]'+JumpFit(2);
% clear trekD JumpCorr JumpInd JumpVal;
...
while (DeltaNPeaks>0.1*NPeaks)&&(Pass<=MaxPass)
  ...
  if SecondPassFull||Pass==1
      ...
      NoiseArray(JumpInd)=true;  % all jumps are noise
      ...
      % divide noise by intervals for faster spline calculations: 
      NoiseArrayR=circshift(NoiseArray,1);   NoiseArrayR(1)=1;     
      NoiseArrayL=circshift(NoiseArray,-1);  NoiseArrayL(end)=1;
      StartNoise=find(NoiseArray-NoiseArrayR==1);   
      EndNoise = find(NoiseArray-NoiseArrayL==1);  % Noise intervals

      
      if  StartNoise(1)<EndNoise(1)       StartNoise(1)=[];   end;
      if  StartNoise(end)<EndNoise(end)   EndNoise(end)=[];   end;
      %In the begining was a noise...
      NoiseArray(1:EndNoise(1))=true; StartNoise=[1;StartNoise];      
      %To the end everyting come back to noise...
      NoiseArray(StartNoise(end):end)=true;  EndNoise=[EndNoise;trekSize];
      SingleNoise=StartNoise==EndNoise;
      NoiseArray(StartNoise(SingleNoise))=false;
      StartNoise(SingleNoise)=[];  EndNoise(SingleNoise)=[]; 
      clear  SingleNoise;
      NoiseArray0=NoiseArray;
      NoiseIntervalN=size(StartNoise,1);   
      fprintf('Spline %7.0f noise points in %5.0f intervals:\n ', size(find(NoiseArray),1),NoiseIntervalN);
      fprintf('contineous noise can be splined very long time !\n');
      
      for i=1:NoiseIntervalN
        NoiseSpline(i)=csaps(trek(StartNoise(i):EndNoise(i),1),trek(StartNoise(i):EndNoise(i),2),0.5);  
      end; 
      fprintf('Spline took                     %7.4f  sec\n', toc); tic;
      
%       NoiseSplined=[]; 
%       for i=1:NoiseIntervalN 
%           NoiseSplined=[NoiseSplined;NoiseSpline(i).coefs(:,end)];%...
%                        % NoiseSpline(i).coefs(end,end)];  
%       end; 
%       fprintf('Noise splined                   %7.4f  sec\n', toc); tic;

      NoiseSplined=[]; 
      for i=1:NoiseIntervalN 
          NoiseSplined=[NoiseSplined;NoiseSpline(i).coefs(:,end);...
                        polyval(NoiseSpline(i).coefs(end,:),tau)];  
      end; 
      fprintf('Noise splined                   %7.4f  sec\n', toc); tic;
       
      NoiseInterp=interp1(trek(NoiseArray,1),NoiseSplined,trek(:,1),'linear');
      fprintf('Liner noise interpolation all points          %7.4f  sec\n', toc); tic;
      
      clear NoiseSplined NoiseSplined NoiseArrayR NoiseArrayL;
      
      trek(:,2)=trek(:,2)-NoiseInterp;
      fprintf('Noise interpolation                   %7.4f  sec\n', toc); tic;
...
%       end;
      ...
      if Plot1;  TrekFigure0=figure;
          ...
          plot(trekStart(not(NoiseArray0),1),trekStart(not(NoiseArray0),2),'b.');
          plot([trek(1,1),trek(end,1)],[StdVal0,StdVal0],'-r','LineWidth',1);
          ...
          plot(trek(not(NoiseArray),1),trek(not(NoiseArray),2),'b.');
      ...
      NoiseIntervalN=StartSignal(2:end)-EndSignal(1:end-1);
      NoiseIntervalN=[StartSignal(1)-1;NoiseIntervalN];
      NoiseIntervalN=[NoiseIntervalN;trekSize-EndSignal(end)];
  ...
  for i=1:SignalN
      ...
      if Pass==1
          ...
          NumGoodPeaks(i)=size(GoodVisiblePeakInd,1);
          ...
          MaxSignInd(i,1)=S+VisiblePeakInd(Ind)-1;
  ...
  DoubleFrontInd=find(FrontSignalN>1.5*MaxFrontN);        
  DoubleFrontSize=size(DoubleFrontInd,2);                 
  % Search first peak in double fronts:                   
  if DoubleFrontSize>0                                    
      for i=1:DoubleFrontSize                             
          S=StartSignal(DoubleFrontInd(i))+1;             
          E=StartSignal(DoubleFrontInd(i))+FrontSignalN(DoubleFrontInd(i))-1;
          PeakSpanInd=find((trekD(S:E)<=trekD(S+1:E+1))&...
              (trekD(S:E)<trekD(S-1:E-1)));               
          if ~isempty(PeakSpanInd)
              PeakInd=[PeakInd;max(S+PeakSpanInd(1)-1,S)];    
              PeakOnFrontInd=[PeakOnFrontInd;max(S+PeakSpanInd(1)-1,S)];
              NumPeaks(DoubleFrontInd(i))=NumPeaks(DoubleFrontInd(i))+1; 
              FrontSignalN(DoubleFrontInd(i))=NumPeaks(DoubleFrontInd(i))+1;
          end;
      end;                                                
  ...
  if Pass==1
      %search signal intervals for standard peaks:
      ...
      StandardPeaks=find((NumPeaks==1)'&(NumGoodPeaks==1)'&...
          ...
          ((MaxSignInd-StartSignal)>=MinFrontN)&...
          ((MaxSignInd-StartSignal)<=MaxFrontN)&...
          (MaxPeak-trek(StartSignal,2)>=3*Threshold)&...
          (MaxPeak-trek(EndSignal,2)>=3*Threshold)&...
          (NoiseIntervalN(2:end)>MaxTailN)&(NoiseIntervalN(1:end-1)>MaxFrontN)&...
          (MaxPeak-trek(StartSignal,2)<MaxSignal)&MaxPeak<MaxSignal);
          
      StandardPeaksN=size(find(StandardPeaks),1);
      plot(trek(MaxSignInd(StandardPeaks),1),trek(MaxSignInd(StandardPeaks),2),'^r');
      %         figure; hold on;
      ...
      AscendTop=trek(MaxSignInd(StandardPeaks)-1,2)>=trek(MaxSignInd(StandardPeaks)+1,2);


      if MaxSignInd(StandardPeaks(end))-AscendTop(end)+SampleTail>trekSize
          StandardPeaksN=StandardPeaksN-1; StandardPeaks(end)=[]; end;
      if MaxSignInd(StandardPeaks(1))-AscendTop(1)-SampleFront<1; 
      ...
      for i=1:StandardPeaksN;
          NN=MaxSignInd(StandardPeaks(i))-AscendTop(i);
      ...
      plot(MeanStandardPulse,'-c.','LineWidth',2,'MarkerSize',12); plot(stdStandardPulse,'-m.');
      ylim([min(StandardPulse(SignalsOk)), 1.1*max(StandardPulse(SignalsOk))]);      
      grid on;      
      
      %remove all siganals too deviated from the MeanStandardPulse:
      SignalsOk=(abs(StandardPulse-repmat(MeanStandardPulse,1,StandardPeaksN))<...
                repmat(2*stdStandardPulse,1,StandardPeaksN))&SignalsOk;
      for i=1:SampleN
          SelectedStandrdPulse=StandardPulse(i,SignalsOk(i,:));
          MeanStandardPulse(i,1)=mean(SelectedStandrdPulse);
          stdStandardPulse(i,1)=std(SelectedStandrdPulse);
      ...
      % final clean of StandardPulse: 
      [MinStdLeft,LeftMin]=min(stdStandardPulse(1:SampleFront));
      stdStandardPulseD=diff(stdStandardPulse);
      RightMin=SampleFront+1+min(find(stdStandardPulseD(SampleFront+2:end)>0));
      MinStdRight=stdStandardPulse(RightMin);
      ...
      x=[1:SampleFront+1-MaxFrontN,SampleN-3:SampleN]';
      ...
      %remove low race front points: 
      MeanStandardPulseD=diff(MeanStandardPulse);
      [MaxRace,MaxRaceInd]=max(MeanStandardPulseD);
      MeanStandardPulseD(MaxRaceInd+1:end)=[];
      LowFrontRace=MeanStandardPulseD<MaxRace/20;
      MeanStandardPulse(LowFrontRace)=0;
      FirstNonZero=max(find(MeanStandardPulse(1:MaxStandardPulseIndx)==0))+1;
      LastNonZero=min(find(MeanStandardPulse(MaxStandardPulseIndx:end)==0))+MaxStandardPulseIndx;
      MeanStandardPulse(1:FirstNonZero-1)=0;
      MeanStandardPulse(LastNonZero+1:end)=0;
      
      plot(MeanStandardPulse,'-c','LineWidth',2);
      for i=1:SampleN
      ...
      xInterp=FitInterpPulseInterval(1):InterpN:FitInterpPulseInterval(end);
  ...
  FitSignalStart=PeakInd+StartFitPoint;
  if FitSignalStart(1)<1 
     FitSignalStart(1)=[]; 
     PeakInd(1)=[]; PeakN=PeakN-1;
  end; 
...
%  PulsePlot=figure;
  while i<PeakN
      ...
      if trekMinus(PeakInd(i),2)-B(i)>Threshold
          ...
          x=[MinKhi2Idx(i)-1:1:MinKhi2Idx(i)+1]';
          if (MinKhi2Idx(i)>1)&(MinKhi2Idx(i)<InterpRange)
              if ShortFit A=Sums2(x)./Sums3Short(x); else A=Sums2(x)./Sums3(x);end;
          end;
          if MinKhi2Idx(i)==1
              KhiEdge=Khi2Fit(1); Khi=0;
              iEdge=-InterpHalfRange;
              Sums3Edge=Sums3;
              while Khi<=KhiEdge
                  iEdge=iEdge-1;
                  PulseInterpShiftedEdge=circshift(PulseInterp(:,2),iEdge);
                  FitPulsesEdge=PulseInterpShiftedEdge(xInterp);
                  ...
                  if ShortFit;   Khi=Khi*KhiCoeffShort;
                  else;  Khi=Khi*KhiCoeff;  end;
                  Khi2Fit=[Khi;Khi2Fit];
                  KhiEdge=Khi2Fit(2);
              end;
              MinKhi2Idx(i)=2;  MinKhi2(i)=Khi2Fit(2);
              x=[MinKhi2Idx(i)-1:1:MinKhi2Idx(i)+1]';
              A=Sums2(x)./Sums3Edge(x);
          end;
          if MinKhi2Idx(i)==InterpRange
              KhiEdge=Khi2Fit(end); Khi=0;
              iEdge=InterpHalfRange;
              Sums3Edge=Sums3;
              while Khi<=KhiEdge
                  iEdge=iEdge+1;
                  PulseInterpShiftedEdge=circshift(PulseInterp(:,2),iEdge);
                  FitPulsesEdge=PulseInterpShiftedEdge(xInterp);
                  Sums3Edge=[Sums3Edge;FitPulsesEdge(1:FitNi)'*FitPulsesEdge(1:FitNi)];
                  Sums2=[Sums2;FitPulsesEdge'*NetFitSignal];
                  Khi=Sums1-(Sums2(end)^2)./Sums3Edge(end);
                  if ShortFit;   Khi=Khi*KhiCoeffShort;
                  else;  Khi=Khi*KhiCoeff;  end;
                  Khi2Fit=[Khi2Fit;Khi];
                  KhiEdge=Khi2Fit(end-1);
              end;
              [MinKhi2(i),MinKhi2Idx(i)]=min(Khi2Fit);
              x=[MinKhi2Idx(i)-1:1:MinKhi2Idx(i)+1]';
              A=Sums2(x)./Sums3Edge(x);
          ...
          MinKhi2(i)=Khi2MinFine;
          Ampl(i)=interp1(x,A,Khi2MinIdxFine,'spline');
...
%           %%%%Check that MinKhi2Idx(i) is within FitPulses(1:FitNi,:)
%           %%%%range!!!!
%           plot([1:FitNi]'+StartFitPoint-1,A(2)*FitPulses(1:FitNi,MinKhi2Idx(i))+B(i),'-g.');
%           [Min,k]=min([Khi2Fit(MinKhi2Idx(i)-1),MinKhi2(i)*1000,Khi2Fit(MinKhi2Idx(i)+1)]);
%           plot([1:FitNi]'+StartFitPoint-1,A(k)*FitPulses(1:FitNi,MinKhi2Idx(i)+k-2)+B(i),'-g.');
%           plot(PulseInterpFine(:,1),Ampl(i)*PulseInterpFineShifted+B(i),'-r');
%           subplot(2,1,2); 
...
%           %%check MinKhi2Idx(i) to be inside the KhiDD index range
%           text(Khi2MinIdxFine,mean(Khi2Fit),num2str(KhiDD(MinKhi2Idx(i))),'color','b');
%           pause(0.05);


          FitIdx=PulseInterpFine(1:FineInterpN:end,1);
          SubtractedPulse=Ampl(i)*PulseInterpFineShifted(1:FineInterpN:end);
          ...
          if (Ampl(i)>Threshold)&((Ampl(i)/StdVal)^2>2*MinKhi2(i)); %(MinKhi2(i)<Khi2Thr)&;
              ...
              peaks(NPeaks,5)=Ampl(i);                           %Peak Amplitude
              peaks(NPeaks,6)=MinKhi2(i);                        % KhiMin
          ...
          end;  %(MinKhi2(i)>0)&(MinKhi2(i)<Khi2Thr)&(Ampl(i)>0);
      end;  %if trekMinus(PeakInd(i),2)-B(i)>Threshold
  end;  %while
  ...
  if Pass==1
      ...
      for i=1:NPeaks
          text(peaks(i,2),peaks(i,5),num2str(i));
          plot(peaks(i,2)+PulseInterpFine(:,1)*tau,...
               peaks(i,4)+peaks(i,5)*PulseInterpFine(:,2),'g');
      end;
      %figure; plot(peaks(:,2),peaks(:,6),'-ko');
  end;
  if Pass==2
  ...
  if Pass==1
      ...
      clear MaxPeak MaxSignInd MaxStPeak MaxStPeakIdx MaxSPInd MaxIndSP MaxSP MinSP MinSPInd MinStPeak;
  end;
